...
#setwd
wd <- 'C:\\Users\\John\\Documents\\msu\\phyllosphere_soil_metacommunity'
setwd(wd)
library(tidyverse)
library(data.table)
library(kableExtra)
...
#load metadata
meta_all <- read.table('Map_GLBRC_16S.txt', sep = '\t') %>% 
  transmute(
    plot = plotID, 
    sp = plant,
    source = source,
...
if (TRUE) {
  ...
  saveRDS(abun_shifts, file = 'abun_shifts.RDS')

}

#load abundance_shift data
abun_shifts <- readRDS('abun_shifts.RDS')
...
if(TRUE) {
  ...
  comms %>% 
    ...
    rename(date = date1) %>%
    select(-date0) %>%
    saveRDS("sim_comms.RDS")

}

comms <- readRDS("sim_comms.RDS")
